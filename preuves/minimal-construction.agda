...
-- ActionMorphismEquiv : {G : Group ℓ} {X : hSet ℓ} → Action G ⟨ X ⟩ ≃ GroupHom G (AutGroup {X = X})
-- ActionMorphismEquiv {G = G} {X = (X , trc)} = isoToEquiv e
--   where
--     open GroupStr (str G) renaming (inv to inv₁)
--     e : Iso (Action G X) (GroupHom G (AutGroup))
--     Iso.fun e ϕ =
--       (λ g → isoToEquiv (leftMul g) ) ,
--       record {
...
--         _*_ = Action._*_ ϕ
--         leftMul : ⟨ G ⟩ → Iso X X
--         Iso.fun (leftMul g) = (_*_) g
--         Iso.inv (leftMul g) = (_*_) (inv₁ g)
--         Iso.rightInv (leftMul g) x = (Action.·Composition ϕ _ _ _) ∙ cong (λ y → y * x) (·InvR g) ∙ Action.·Unit ϕ _
--         Iso.leftInv (leftMul g) x = (Action.·Composition ϕ _ _ _) ∙ cong (λ y → y * x) (·InvL g) ∙ Action.·Unit ϕ _
...
actionToMorphism {G = G} {X = (X , trc)} ϕ = (λ g → leftMul ϕ g ) ,
  record {
    pres· = λ x y → ΣPathP ((funExt (λ z → sym (Action.·Composition ϕ _ _ _))) , toPathP (isPropIsEquiv _ _ _)) ;
    pres1 = ΣPathP ((funExt (Action.·Unit ϕ)) , toPathP (isPropIsEquiv _ _ _)) ;
    presinv = λ x → ΣPathP ({!!} , {!!})
...
module _ (G : Group ℓ) (n : ℕ) (g : Fin n → ⟨ G ⟩) where
  ...
  isContrCommutingSquaresAction (X , trc) f decomp = Cubical.HITs.PropositionalTruncation.elim (λ _ → isPropIsContr) (λ comSq → unique_action comSq)
    ...
    where
      ...
      unique_action comSq = (generateAction , generatedActionCommutesSquares) , isPropCommutingSquareAction
        where
        ...
        HomHDecomp = (λ x → compEquiv hEq (compEquiv (EquivLeftMul (prod x)) (invEquiv hEq))) ,
          record {
            pres· = λ x y →
             ...
             compEquiv hEq (compEquiv (EquivLeftMul (prod x ·₁ (prod y))) (invEquiv hEq)) ≡⟨ cong ((λ x → compEquiv hEq (compEquiv x (invEquiv hEq)))) EquivLeftMulPres· ⟩
             compEquiv hEq (compEquiv (compEquiv (EquivLeftMul (prod y)) (EquivLeftMul (prod x))) (invEquiv hEq)) ≡⟨ cong {!!} {!!} ⟩
        ...
        generatedActionCommutesSquares i x =
          (generateAction Action.* g i) x ≡⟨ decompose ⟩
        ...
        isPropCommutingSquareAction (ϕ , pϕ) = ΣPathP (equalActions generateAction ϕ (funExt λ g' → (funExt λ x →
                                    ...
                                    where
                                      ...
                                      wordToAction = Cubical.HITs.FreeGroup.rec λ i → fst (actionToMorphism ϕ) (g i)
                                      ...
                                      lem = Cubical.HITs.FreeGroup.elimProp {B = λ w' → (x' : X) → fst (fst wordToAction w') x' ≡ (prod w') *₁ x'}
                                        ...
                                        (λ w1 w2 p1 p2 x' →
                                          ...
                                          (prod w1 ·₁ prod w2) *₁ x' ≡⟨ cong ((λ x → x *₁ x')) ((snd prodHom) .IsGroupHom.pres· _ _) ⟩
                                          prod (w1 ·₂ w2) *₁ x' ∎)
                                        (λ x' →
  ...
  zeBigTheorem gen = isoToEquiv e
    ...
    where
      ...
      Iso.fun e (X , (f , comSq)) = Xgset , equalsPG comSq
        ...
        where
          Xgset : GSet ℓ G
          Xgset = (⟨ X ⟩ , gsetstr (fst (fst (isContrCommutingSquaresAction X f gen comSq))))
          ...
          equalsPG = Cubical.HITs.PropositionalTruncation.elim (λ _ → isPropPropTrunc) λ comSq → ∣ fst theorem (gse comSq) ∣₁
            ...
            where
              ...
              gse (hEq , com) = invEquiv hEq , isGSetHomInv hEq (record { pres* = {!!} })

      Iso.inv e (Xgset , equalsPG) = (⟨ Xgset ⟩ ,  trc) , (λ i → leftMul ϕ (g i)) , {!!}

        where
          _*_ = (str Xgset) .GSetStr._*_
          trc = (str Xgset) .GSetStr.is-set
          -- How do you extract the action ?
          ϕ = action _*_ trc (str Xgset .GSetStr.·Unit) (str Xgset .GSetStr.·Composition)
