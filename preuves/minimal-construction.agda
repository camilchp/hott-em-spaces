  ...
  isContrCommutingSquaresAction (X , trc) f decomp = Cubical.HITs.PropositionalTruncation.elim (λ _ → isPropIsContr) (λ comSq → unique_action comSq)
    ...
    where
      ...
      unique_action comSq = (generateAction , generatedActionCommutesSquares) , isPropCommutingSquareAction
        ...
        open decomposition-tools (X , trc) f comSq
  ...
  zeBigTheorem decomp = isoToEquiv e
    ...
    where
      ...
      Iso.fun e (X , (f , comSq)) = Xgset , equalsPG comSq
        where
          Xgset : GSet ℓ G
          Xgset = (⟨ X ⟩ , gsetstr (fst (fst (isContrCommutingSquaresAction X f decomp comSq))))

          gse : commutingSquares X f → (GSetEquiv PG Xgset)
          gse comSq = invEquiv hEq , isGSetHomInv hEq (record { pres* = λ g' x →
                h (str Xgset .GSetStr._*_ g' x) ≡⟨ {!refl!} ⟩
      ...
      Iso.inv e (Xgset , equalsPG) = (⟨ Xgset ⟩ ,  trc) , (λ i → leftMul {X = (⟨ Xgset ⟩ , str Xgset .GSetStr.is-set} ϕ (g i)) , {!!}
        where
